...
module.exports = function (thorin, opt) {
  ...
  const Connection = initConnection(thorin);
  ...
  class ThorinRedisStore extends thorin.Interface.Store {
    ...
    * */
    retryConnect() {
      ...
      if (!this._retryCounter) this._retryCounter = 1;
      if (this._retryConnect) clearTimeout(this._retryConnect);
      if (this[config].debug) {
        logger.trace(`Re-trying to connect to redis [${this._retryCounter}]`);
      }
      cObj.connect((e) => {
        if (!e) {
          logger.info(`Re-connected to redis`);
          delete this._retryConnect;
          delete this._retryCounter;
          return;
        }
        this._retryCounter++;
        this._retryConnect = setTimeout(() => {
          this.retryConnect();
        }, retryInterval);
      });
    }
