...
module.exports = function init(thorin) {
  ...
  class RedisConnection extends EventEmitter {
    ...
    connect(done, _fromCluster) {
      ...
      if (this.connection) {
        if(this.#config.clustered) {
          this.connection = new Redis.Cluster([{host: _opt.host}],{dnsLookup: _opt.dnsLookup});
        }
    ...
    startSubscriber() {
      ...
      this.connection.on('pmessage', this.handlePatternMessage.bind(this));
    }

    handlePatternMessage(pattern, channel, msg) {
      if (!this.#subscribers) return;
      if (typeof this.#subscribers[pattern] === 'undefined' || this.#subscribers[pattern].length === 0) return;
      let fns = this.#subscribers[pattern];
    ...
    handleSubscribe(channel, fn) {
      ...
      if (!this.#subscribers[channel]) {
        if (channel.indexOf('*') !== -1) {
          this.connection.psubscribe(channel);
        } else {
    ...
    handleUnsubscribe(channel, _fn) {
      ...
      if (typeof _fn !== 'function') {
        ...
        for (let i = 0; i < this.#subscribers[channel].length; i++) {
          if (this.#subscribers[channel][i] === _fn) {
